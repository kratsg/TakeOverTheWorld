...
class HistsContainer(list):
  def __init__(self, iterable=[]):
    super(self.__class__, self).__init__(filter(lambda x: isinstance(x, HistsFile), iterable))

  def append(self, item):
    if not isinstance(item, HistsFile): return
    return super(self.__class__, self).append(item)

  def extend(self, iterable):
    return super(self.__class__, self).extend(filter(lambda x: isinstance(x, HistsFile), iterable))
